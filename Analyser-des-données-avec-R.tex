...
\StringTok{     }\KeywordTok{geom_text}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{label =}\NormalTok{ n), }\DataTypeTok{size =} \DecValTok{2}\NormalTok{, }\DataTypeTok{nudge_y =} \DecValTok{700}\NormalTok{) }\OperatorTok{+}
...
\section[Relation entre une variable quantitative et une variable qualitative]{Relation entre une variable quantitative et une variable qualitative%
\sectionmark{Relation entre une variable quantitative et une variable qual... }}
\sectionmark{Relation entre une variable quantitative et une variable qual... }
...
La loi binomiale est une loi mathématique qui concerne les situations où seulement deux résultats sont possibles, comme par exemple \enquote{succès} et \enquote{échec}, \enquote{pile} et \enquote{face}, \enquote{0} et \enquote{1}, etc. Un exemple classique où l'on est en présence d'une variable binomiale est celui où l'on demande à \(N\) personnes de lancer une fois une pièce non truquée, avec par conséquent \enquote{pile} et \enquote{face} comme seuls résultats possibles et autant de chances de tomber sur \enquote{pile} que sur \enquote{face} à chaque lancer (\(\theta\) = 0.5, soit une chance sur deux). Une fois les essais de toutes les personnes terminés, la distribution qui en résulte contient alors la proportion de personnes qui ont obtenu \enquote{pile} et la proportion de personnes qui n'ont pas obtenu \enquote{pile} (donc \enquote{face}). Lorsque le nombre \(N\) d'essais ou de participants et la probabilité d'avoir tel ou tel résultat (\(\theta\)) sont connus, la loi binomiale, qu'on note \(X \sim B(\theta, N)\), permet de connaître la probabilité \(P(X)\) d'avoir \(X\) succès (e.g., \(X\) fois \enquote{pile}) sur les \(N\) essais ou participants. La formule de cette loi est montrée ci-dessous :
...
\section[Loi des grands nombres, distribution d'échantillonnage de la moyenne, et théorème de la limite centrale]{Loi des grands nombres, distribution d'échantillonnage de la moyenne, et théorème de la limite centrale%
\sectionmark{Loi des grands nombres, distribution d'échantillonnage...}}
\sectionmark{Loi des grands nombres, distribution d'échantillonnage...}
...
\section[Test du \texorpdfstring{$\chi^2$}{[chi-carré]} d'adéquation (ou encore dit de conformité ou d'ajustement)]{Test du \texorpdfstring{$\chi^2$}{[chi-carré]} d'adéquation (ou encore dit de conformité ou d'ajustement)%
\sectionmark{Test du \texorpdfstring{$\chi^2$}{[chi-carré]} d'adéquation...}}
\sectionmark{Test du \texorpdfstring{$\chi^2$}{[chi-carré]} d'adéquation...}
...
\NormalTok{observed <-}\StringTok{ }\KeywordTok{table}\NormalTok{(cartes}\OperatorTok{$}\NormalTok{choix)}
\NormalTok{observed}
\end{Highlighting}
\end{Shaded}
...
\NormalTok{cartes }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{group_by}\NormalTok{(choix) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{count}\NormalTok{(choix) }\OperatorTok{%>%}\StringTok{ }
\StringTok{  }\KeywordTok{ggplot}\NormalTok{(}\KeywordTok{aes}\NormalTok{(}\DataTypeTok{x =} \KeywordTok{fct_reorder}\NormalTok{(choix, n), }\DataTypeTok{y =}\NormalTok{ n)) }\OperatorTok{+}\StringTok{ }
\StringTok{  }\KeywordTok{geom_bar}\NormalTok{(}\DataTypeTok{stat =} \StringTok{"identity"}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
{\centering \includegraphics[width=0.8\linewidth]{Analyser-des-données-avec-R_files/figure-latex/unnamed-chunk-14-1} 
...
\caption{Distribution d'échantillonnage de la statistique $\chi^2$ sous $H{0}$}\label{fig:GOFDistri}
\end{figure}
...
\begin{Highlighting}[]
\NormalTok{observed <-}\StringTok{ }\KeywordTok{table}\NormalTok{(cartes}\OperatorTok{$}\NormalTok{choix)}
\NormalTok{expected <-}\StringTok{ }\KeywordTok{c}\NormalTok{(}\FloatTok{0.25}\NormalTok{, }\FloatTok{0.25}\NormalTok{, }\FloatTok{0.25}\NormalTok{, }\FloatTok{0.25}\NormalTok{)}
\KeywordTok{chisq.test}\NormalTok{(}\DataTypeTok{x =}\NormalTok{ observed, }\DataTypeTok{p =}\NormalTok{ expected)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lsr}\OperatorTok{::}\KeywordTok{goodnessOfFitTest}\NormalTok{(}\KeywordTok{as.factor}\NormalTok{(cartes}\OperatorTok{$}\NormalTok{choix), }\DataTypeTok{p =} \KeywordTok{c}\NormalTok{(}\FloatTok{0.25}\NormalTok{, }\FloatTok{0.25}\NormalTok{, }\FloatTok{0.25}\NormalTok{, }\FloatTok{0.25}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{lsr}\OperatorTok{::}\KeywordTok{goodnessOfFitTest}\NormalTok{(}\KeywordTok{as.factor}\NormalTok{(cartes}\OperatorTok{$}\NormalTok{choix), }\DataTypeTok{p =} \KeywordTok{c}\NormalTok{(}\FloatTok{0.05}\NormalTok{, }\FloatTok{0.8}\NormalTok{, }\FloatTok{0.05}\NormalTok{, }\FloatTok{0.1}\NormalTok{))}
\end{Highlighting}
\end{Shaded}
