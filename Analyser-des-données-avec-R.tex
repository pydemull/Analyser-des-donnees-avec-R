...
{\centering \includegraphics[width=0.95\linewidth]{Analyser-des-données-avec-R_files/figure-latex/graphCov-1} 
...
Pour reporter les résultats, nous pourrions écrire les choses comme cela \autocite{navarroLearningStatistics2018} : Le résultat du test était significatif (\(\chi^2(3)\) = 8.44, \(P\) = 0.04).

Il convient de noter que les probabilités espérées (théoriques) peuvent être définies à volonté selon la question de recherche. Par exemple, on aurait pu tester l'hypothèse \(H_{0}\) selon laquelle les personnes préfèrent à 80 \% les coeurs, 10 \% les trèfles, 5 \% les piques, et 5 \% les carreaux. Le tout est de bien configurer le test pour que les fréquences observées pour chaque modalité (ici les types de cartes) soient bien mises en correspondance avec les fréquences théoriques (cf.~argument \texttt{p}dans l'exemple de code ci-dessous). La fonction \texttt{lsr::goodnessOfFitTest()} a donc un fort intérêt ici pour bien vérifier, dans les résultats affichés, qu'on a bien attribué les probabilités théoriques aux bonnes modalités) :
