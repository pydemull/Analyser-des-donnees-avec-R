...
```{r geomhistogram, fig.align ="center", fig.cap = "Exemple d'histogramme"}
...
iris %>% 
  ggplot(aes(x = Sepal.Length)) +                  # On remarque ici que l'argument data a dû être enlevé.
  geom_histogram(fill = "white", color = "black")
```
...
```{r geomhistogramVariedBinwidth2, echo = FALSE, fig.align ="center",  fig.cap="Différentes largeurs d'intervalles pour un histogramme"}
...
```{r geomboxplot, fig.align ="center", fig.cap="Exemple de boîte à moustaches"}
ggplot(data = iris, aes(x = Sepal.Length)) + 
  geom_boxplot() +
...
```{r geomboxplotModified, fig.align = "center", fig.cap="Visualisation d'un outlier"}
iris$Sepal.Length[3] <- 12                     # On modifie ici, pour l'exemple, la valeur de la 3ème observation 
...
Une fois cette procédure réalisée, le reste du code, et notamment la fonction `geom_text()`, a permis d'ajouter des éléments textuels au graphique, en l'occurence en s'appuyant sur la variable id_outlier, tel que configuré avec l'argument `aes()` de la fonction `geom_text()`. Lorsque la valeur anormale identifiée est effectivement une erreur de saisie dans la base de données, il convient de corriger la valeur avec la fonction d'assignation comme nous l'avons fait précédemment :

```{r outlier correction}
iris$Sepal.Length[3] <- 4.7  # Le nombre entre crochets désigne la position de l'observation dans la variable.
```
...
TextPlot_gauss <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 5, label = "Gaussienne") +
            theme(
...
TextPlot_skew_right <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 5, label = "Asymétrique") +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(), 
...
TextPlot_lepto <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 5, label = "Leptocurtique") +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(), 
...
TextPlot_plati <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 5, label = "Platycurtique") +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(), 
...
gauss_g <- g_distri(x = gauss)
skew_right_g <- g_distri(x = skew_right)
lepto_g <- g_distri(x = lepto) +  coord_cartesian(xlim = c(-5, 5))
plati_g <- g_distri(x = plati)

(TextPlot_gauss | TextPlot_skew_right) /
(gauss_g | skew_right_g) /
(TextPlot_lepto | TextPlot_plati) /
(lepto_g | plati_g) +
plot_layout(heights = c(0.3, 1, 0.3, 1))
```
...
```{r mediansSkewedVariables, echo = FALSE, message = FALSE, warning = FALSE, fig.align ="center", fig.cap = "Médiane d'une variable présentant une distribution asymétrique"}
# Graphique pour la distribution gaussienne
set.seed(123)
gauss <- tibble(x = rnorm(n = 1000, mean = 0, sd = 1))
mean_gauss <- mean(gauss$x)
...
Le mode désigne la valeur qui est la plus fréquemment retrouvée dans une variable. Il n'existe pas de fonction de base dans R pour pouvoir déterminer directement le mode et pour connaître le nombre de fois que le mode apparaît dans la variable. Toutefois, nous pouvons utiliser le package `lsr` crée par Danielle Navarro [-@Navarro2018] pour retrouver ces informations en présence d'une variable quantitative. Une fois le package `lsr` installé puis chargé, nous pouvons utiliser la fonction `modeOf()` pour déterminer le mode, et la fonction `maxFreq()` pour savoir à quelle fréquence revient le mode dans la variable.

```{r mode}
library(lsr)
modeOf(x = iris$Sepal.Length)                     # Détermination du mode.
maxFreq(x = iris$Sepal.Length)                    # Détermination de la fréquence du mode.
```
...
La démarche demeure la même pour déterminer Q3, à ceci près qu’on utilise le nombre 3*N* et non plus le nombre *N* pour les calculs [@Labreuche2010]. Cette méthode de calcul, classique, est en principe à privilégier en présence d'une variable discrète. Si l'on souhaite obtenir les quartiles selon cette méthode avec le logiciel R, il faut utiliser la fonction `quantile()` de la manière suivante : 
...
knitr::kable(
table_quant,
caption = "Quantiles d'une variable quantitative continue"
)
```
...
Comme on peut le voir dans l'aide associée à la fonction `skewness()`, il existe en réalité plusieurs méthodes de calcul du coefficient d'asymétrie. La méthode de `type` 3, qui est celle configurée par défaut pour cette fonction, consiste à faire le calcul suivant :
...
```{r skeweddistri, echo = FALSE, message = FALSE, warning = FALSE, fig.align ="center", fig.cap="Valeur du Skewness selon la forme de la distribution"}
set.seed(123)
...
TextPlot_chisq1 <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 5, label = bquote(paste(gamma[1], " = ", .(round(skewness(chis_1$x), 2))))) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(), 
...
TextPlot_chisq4 <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 5, label = bquote(paste(gamma[1], " = ", .(round(skewness(chis_4$x), 2))))) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(), 
...
TextPlot_gauss <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 5, label = bquote(paste(gamma[1], " = ", .(round(skewness(gauss$x), 2))))) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(), 
...
TextPlot_chisq4_neg <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 5, label = bquote(paste(gamma[1], " = ", .(round(skewness(chis_4_neg$x), 2))))) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(), 
...
TextPlot_chisq1_neg <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 5, label = bquote(paste(gamma[1], " = ", .(round(skewness(chis_1_neg$x), 2))))) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(), 
...
Joanes et Gill [-@Joanes1998] ont montré que lorsqu'il s'agit d'estimer le degré d'asymétrie de la distribution relative à la population étudiée, et cela à partir de l'échantillon observé, certaines méthodes de calcul du coefficient d'asymétrie peuvent être plus fiables que d'autres. 
...
```{r kurtosisdistri, echo = FALSE, message = FALSE, warning = FALSE, fig.align ="center", fig.cap="Valeur du Kurtosis selon la forme de la distribution"}
...
TextPlot_unif <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 5, label = bquote(paste(gamma[2], " = ", .(round(kurtosis(unif$x), 2))))) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(), 
...
TextPlot_gauss <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 5, label = bquote(paste(gamma[2], " = ", .(round(kurtosis(gauss$x), 2))))) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(), 
...
TextPlot_laplace <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 5, label = bquote(paste(gamma[2], " = ", .(round(kurtosis(laplace$x), 2))))) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
              panel.grid.minor = element_blank(), 
...
*Pour aller plus loin...*
Comme avec le coefficient d'asymétrie, Joanes et Gill [-@Joanes1998] ont montré que lorsqu'il s'agit d'estimer le degré d'aplatissement de la distribution relative à la population étudiée, et cela à partir de l'échantillon observé, certaines méthodes de calcul du coefficient d'aplatissement peuvent être plus fiables que d'autres. 
...
```{r ObservationsWithinSdMutiplesGaussianDistri, echo = FALSE, warning = FALSE, message = FALSE, fig.align ="center", fig.cap="Proportions des observations incluses dans différents intervalles liés à des multiples de l'écart-type"}
set.seed(123)
...
Lorsqu'il s'agit plus précisément de vouloir renseigner sur la tendance centrale relative à l'échantillon étudié, le choix dépend de la forme de la distribution observée. Lorsque la distribution est gaussienne, la moyenne, la médiane, et le mode, sont similaires et donc se valent. Toutefois, lorsque la distribution est asymétrique et unimodale (i.e., avec un seul pic), le mode reflètera mieux la tendance centrale. De plus, lorsque la distribution est asymétrique, la médiane aura tendance à mieux représenter la tendance centrale que la moyenne [@Rousselet2020]. Notons que dans certains cas où la distribution semble asymétrique, la médiane peut se retrouver malgré tout plus éloignée du mode que la moyenne, comme illustré dans l'exemple ci-dessous emprunté à Gonzales et al. [-@Gonzales2001].

```{r MeanAboveLedianAsym, echo = FALSE, warning = FALSE, message = FALSE, fig.align ="center", fig.cap = "Moyenne et médiane dans le cadre d'une distribution asymétrique"}
...
```{r geombar, fig.align ="center", fig.cap="Exemple de diagramme en barres"}
...
```{r geombarEnhancedGraph, echo = FALSE, message = FALSE, fig.align ="center",  fig.cap="Différentes sortes de diagrammes en barres"}
(g1 | g2 | g3) / (g4 | g5 | g6)
```
...
Toujours dans le code montré ci-dessus, on peut voir aussi, dans la fonction `geom_text()` du graphique g1, la présence de l'argument `nudge_y`. Cet argument permet de régler le décalage entre le haut de la barre et le texte. L'ordre de grandeur du chiffre indiqué est celui de la variable indiquée en ordonnées.
...
Bien que l'on ait l'habitude d'utiliser des graphiques montrant les données de la variable quantitative sur l'axe vertical, il est possible de montrer les données sous formes d'aires de densité le long de l'axe horizontal (cf. graphique g13 ci-dessous) ou encore avec un graphique en lignes de crêtes (*ridgelines plot*, cf. graphique g14 ci-dessous). Ces types de graphiques et leurs intérêts ont été discutés par Wilke [-@Wilke2018]. Pour pouvoir faire le graphque g14, il faut installer et charger un nouveau package : `ggridges`.
...
Une fois que l'on a calculé une taille d'effet, il est toujours intéressant d'essayer de formuler un jugement sur l'importance, l'ampleur de l'effet. En ce sens, des valeurs seuils ont été proposées dans la littérature (Cohen, 1988; in Lakens [-@Lakens2013]). Ces valeurs, valables pour interpréter des tailles d'effet dans le cadre d'une étude de type *between-subject design*, sont montrées dans le tableau ci-dessous.
...
Ces valeurs sont relativement arbitraires, et l'interprétation de la taille de l'effet et des conséquences qu'il peut avoir en pratique ne devrait pas être liée de manière trop rigide à ces valeurs [@Lakens2013].

Il existe également une autre approche pour interpréter une valeur de taille d'effet : l'approche *Common Language explanation* [@Lakens2013]. Cette approche consiste à faire le lien entre la valeur de la taille d'effet et les probabilités de rencontrer des valeurs similaires ou supérieures dans les groupes comparés. Par exemple, lorsqu'on obtient un $d_{s}$ de Cohen de 0.80, cela peut se traduire par le fait qu'il y a 71.4 % de chances qu'une personne prise au hasard dans le groupe avec la meilleure moyenne ait un score plus élevé qu'une personne qui serait prise au hasard dans le groupe avec la moyenne la plus basse des deux groupes. Kristoffer Magnusson a réalisé une page web qui permet d'utiliser l'approche *Common Language explanation* pour n'importe quelle valeur de taille d'effet dans le cadre d'une étude de type *between-subject design*. Jetez un oeil ici : https://rpsychologist.com/cohend.

**gs de Hedges**

Il a été rapporté, dans la littérature, qu'en réalité le $d_{s}$ de Cohen est relativement biaisé lorsqu'il s'agit d'estimer la taille d'un effet dans la population d'intérêt à partir d'échantillons de population (autrement dit, à l'échelle de nombreux échantillons, on sait qu'avec le $d_{s}$ de Cohen on aura en moyenne un écart entre l'effet qu'on a trouvé et l'effet réel qui existe dans la population). Cela sera d'autant plus vrai lorsque les calculs seront réalisés à partir de petits échantillons ($N$ < 20), selon Hedges et Okins (1985) cités par Lakens [-@Lakens2013]. En raison de cela, un autre indice statistique a été proposé pour corriger cette erreur systématique qui sera d'autant plus grande que l'échantillon étudié est petit, à savoir le $g_{s}$ de Hedges :
...
Le calcul classique pour obtenir la taille d'effet désignant l'écart entre deux groupes de données dépendantes est celui du $d_{z}$ [@Lakens2013], qui est montré ci-dessous :
...
Toutefois, comme le rappelle Lakens [-@Lakens2013], le $g_{av}$ de Hedges ne serait pas complètement non biaisé.
...
```{r graphLinStats, fig.align = "center", message = FALSE, warning = FALSE, fig.cap = "Régression linéaire avec les informations correspondantes"}
ggplot(data = mtcars, aes(x = hp, y = mpg)) +
  geom_point() +
  geom_smooth(method = "lm", se = FALSE) +
...
```{r anscombeQuartetGraph, echo = FALSE, fig.align = "center", fig.cap="Le quartet d'Anscombe"}
size <- 3
color <- "red"
