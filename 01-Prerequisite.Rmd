...
### Manipuler des valeurs, des vecteurs, et des tableaux de données
...
L'une des particularités de R, c'est de permettre de stocker des valeurs, des vecteurs, ou encore des tableaux de données, dans des objets. Pour ce faire, R utilise la fonction d'assignation `<-`. Cette fonction s'utilise en écrivant à droite de la flèche ce que l'on veut stocker dans l'objet, et en écrivant à gauche de la flèche le nom que l'on souhaite donner à l'objet. (Attention : Toujours utiliser seulement des caractères alphanumériques et des points `.` ou des tirets du bas _ pour écrire un nom d'objet ; ne pas commencer par un chiffre ; avoir à l'esprit que R est sensible à la casse, ce qui veut dire qu'un objet dont le nom commence par une majuscule sera un objet différent de celui qui a le même nom mais qui  commence par une minuscule.) L'utilisation d'objets permet de grandement faciliter les analyses par la suite. Lorsqu'on réalise une assignation, il est possible de voir le nouvel objet crée dans la fenêtre **Environnement** de RStudio. Lorsqu'un objet est crée via une assignation, R ne montre pas ce qu'il contient. Pour voir cela, il faut écrire le nom de l'objet dans une ligne de code et lancer la commande.

Il est donc possible de stocker dans un objet une valeur numérique...
...
Lorsqu'un objet contenant un tableau de données est crée, il est possible d'afficher le contenu d'une seule colonne de ce tableau, grâce au symbole `$`.

```{r column object}
df$Taille
```

Une fois que des objets sont crées, il est possible, comme montré initialement avec des valeurs, de les utiliser pour réaliser des opérations. Par exemple, on peut manipuler des objets contenant simplement une valeur numérique...
...
Lorsque plusieurs objets ont été crées, il est possible de vouloir en supprimer, par exemple en raison du fait qu'un objet aurait été crée par erreur. Pour supprimer un objet, il est possible d'utiliser la fonction `rm()`. L'instruction `rm(list = ls())` supprime tous les objets qui ont été crées auparavant.
...
* Pour créer un objet, utiliser la fonction d'assignation `<-`.
...
* Un code peut être écrit directement dans la **Console**, ou dans une fenêtre **Script**. Pour activer le code à partir d'un script, utiliser le bouton **Run** ou **Ctrl + Entrée**. Les objets crées apparaissent dans la fenêtre **Environnement**.
* Pour écrire des titres et des commentaires dans un script, utiliser un `#` avant l'écriture du code.
* Dans la Console, le symbole `>` signigie que le logiciel est prêt à lancer une nouvelle instruction. Le symbole `+` indique que l'instruction initialement lancée est incomplète. Mieux vaut alors faire **Echap**, modifier le code, et recommencer.
