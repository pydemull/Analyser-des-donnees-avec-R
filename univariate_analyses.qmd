...
* Une boîte à moustaches, qui permet de se faire une idée résumée relativement robuste de la distribution dans la population d'intérêt (i.e., insensible aux *outliers* et davantage comparable avec d'autres études) ; selon les cas, d'autres statistiques, comme les moyennes et les écarts-types, peuvent être utilisées à la place ou en plus des boîtes à moustaches [@allenRaincloudPlotsMultiplatform2019].
...
rain_cloud <- function(data, x, y) {
  ...
  ggplot(data = data, aes(x = x, y = .data[[y]])) +
    ggdist::stat_halfeye(
    ...
    geom_boxplot(width = .1,
                 outlier.shape = NA) +
    geom_point(aes(x = stage(x, after_scale = x - 0.2)), position = position_jitter(0.05), alpha = 0.2) +
...
TextPlot_chisq1 <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 4,   label = sprintf("gamma[1] == %s", format(round(e1071::skewness(chis_1$x), 2), nsmall = 2)),
    parse = TRUE) +
            theme(
...
TextPlot_chisq4 <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 4,   label = sprintf("gamma[1] == %s", format(round(e1071::skewness(chis_4$x), 2), nsmall = 2)),
    parse = TRUE) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
...
TextPlot_gauss <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 4,   label = sprintf("gamma[1] == %s", format(round(e1071::skewness(gauss$x), 2), nsmall = 2)),
    parse = TRUE) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
...
TextPlot_chisq4_neg <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 4,   label = sprintf("gamma[1] == %s", format(round(e1071::skewness(chis_4_neg$x), 2), nsmall = 2)),
    parse = TRUE) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
...
TextPlot_chisq1_neg <- 
            ggplot() + 
            annotate(geom = "text", x = 4, y = 1, size = 4,   label = sprintf("gamma[1] == %s", format(round(e1071::skewness(chis_1_neg$x), 2), nsmall = 2)),
    parse = TRUE) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
...
TextPlot_unif <- 
ggplot() + 
annotate(geom = "text", x = 4, y = 1, size = 4,   label = sprintf("gamma[2] == %s", format(round(e1071::kurtosis(unif$x), 2), nsmall = 2)),
    parse = TRUE) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
...
TextPlot_gauss <- 
ggplot() + 
annotate(geom = "text", x = 4, y = 1, size = 4,   label = sprintf("gamma[2] == %s", format(round(e1071::kurtosis(gauss$x), 2), nsmall = 2)),
    parse = TRUE) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
...
TextPlot_laplace <- 
ggplot() + 
annotate(geom = "text", x = 4, y = 1, size = 4,   label = sprintf("gamma[2] == %s", format(round(e1071::kurtosis(laplace$x), 2), nsmall = 2)),
    parse = TRUE) +
            theme(
              plot.background = element_blank(), 
              panel.grid.major = element_blank(),
...
```{r describe_data_frame_by_group}
#|  message: false
#|  warning: false
...
```{r ObservationsWithinSdMutiplesGaussianDistri}
#| out.width: '90%'
#| echo: false
#| warning: false
#| message: false
#| fig.cap: "Proportions des observations incluses dans différents intervalles liés à la moyenne et à des multiples de l'écart-type"
#| label: fig-ObservationsWithinSdMutiplesGaussianDistri
...
g1 <- 
  ...
  theme_classic() +
  labs(fill = "") +
  annotate(geom = "text", label =  "bar(italic(X)) %+-% 1*hat(sigma)", 
           x = 15, y = 0.06, size = 5,
           parse = TRUE
           ) +
...
g2 <- 
  ...
  theme_classic() +
  labs(fill = "") +
  annotate(geom = "text", label =  "bar(italic(X)) %+-% 2*hat(sigma)", 
           x = 15, y = 0.06, size = 5,
           parse = TRUE
           ) +
  theme(axis.text = element_blank(),
...
g3 <- 
  ...
  theme_classic() +
  labs(fill = "") +
  annotate(geom = "text", label =  "bar(italic(X)) %+-% 3*hat(sigma)", 
           x = 15, y = 0.06, size = 5,
           parse = TRUE
           ) +
  theme(axis.text = element_blank(),
...
```{r MeanAboveLedianAsym}
#| out.width: '90%'
#| echo: false
#| warning: false
#| message: false
#| fig.cap: "Exemples de positions de la moyenne et de la médiane dans le cadre d'une distribution asymétrique"
#| label: fig-MeanAboveLedianAsym
...
```{r geombar}
#| out.width: '80%'
#| fig.cap: "Exemple de diagramme en barres"
#| label: fig-geombar
...
```{r geombarPropEnhancedGraph}
#| out-width: '100%'
#| echo: false
#| message: false
#| label: fig-geombarPropEnhancedGraph
#| fig.cap: "Différentes sortes de diagrammes pour représenter des proportions"
...
* En statistiques, la notion de population désigne l'ensemble des individus existants qui présentent un ou plusieurs critères d'intérêt. Un échantillon est alors une fraction de la population étudiée, composée d'individus qui en principe sont représentatifs de la population étudiée.
