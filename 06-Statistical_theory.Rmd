...
ggplot(data = binom, aes(x = x)) + 
  ...
  labs(x = "Nombre fois où \"pile\" a été obtenu sur 100 lancers de pièce (x)", y = "Proportion d'individus ayant eu X fois \"pile\" : P(X)") +
  theme_bw()
```
...
* La loi *t* : Les distributions relatives à des lois *t* ressemblent aux distributions relatives à des lois normales mais avec des plus queues plus épaisses. Un distribution *t* peut être obtenue en divisant les valeurs d'une distribution $X^2$ par le nombre de degrés de liberté $k$, puis en prenant leurs racines carrées, et enfin en divisant les valeurs d'une loi normale par la variable obtenue. On obtient alors une distribution *t* à $k$ degrés de liberté.
