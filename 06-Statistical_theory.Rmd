...
ggplot(data = binom, aes(x = x, y  = prob)) + 
  ...
  labs(x = "Nombre de fois où \"pile\" a été obtenu sur 100 lancers de pièce (x)", y = "Proportion d'individus ayant eu \"pile\" : P(X)") +
  theme_bw()
```
...
# à partir de N essais/personnes (ici 100) et d'une probabilité de succès donnée (ici 0.5)
dbinom(x = 50, size = 100, prob = 0.5)

# Fonction pour déterminer la probabilité d'obtenir un nombre de succès inférieur
# ou égal à q (ici 50) à partir de N essais/personnes (ici 100) et d'une probabilité de 
...
# d'obtenir une valeur inférieure ou égale à partir de N essais/personnes (ici 100) et 
