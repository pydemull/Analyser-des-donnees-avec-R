...
\includegraphics[width=0.9\textwidth,height=\textheight]{prerequisite_files/figure-pdf/fig-plot_function2-1.pdf}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{prerequisite_files/figure-pdf/fig-plot_function5-1.pdf}
...
\chapter{Importation et manipulation d'une base de
données}\label{importation-et-manipulation-dune-base-de-donnuxe9es}

\section{Comprendre ce qu'est une base de
données}\label{comprendre-ce-quest-une-base-de-donnuxe9es}

Lorsqu'on souhaite répondre à une question, la démarche scientifique
classique consiste à effectuer une série de mesures ou d'observations
selon un protocole qui a été conçu en cohérence avec la question posée.
En principe, ces mesures ou observations donnent lieu à l'obtention de
valeurs. Ces valeurs peuvent être de forme numérique (e.g., les valeurs
de taille de différents individus) ou de forme littérale (e.g., les
valeurs de sexe de différents individus). Quelle que soit leur forme,
les valeurs que l'on obtient dans un contexte qui est connu, comme dans
le cas d'un protocole de mesures, ont un sens bien défini car elles sont
associées à des choses que l'on a cherché à caractériser. Lorsqu'une
valeur est porteuse d'un sens bien défini, on peut alors considérer
qu'il s'agit d'une \textbf{donnée}. Très souvent, pour répondre à une
question, il est nécessaire d'acquérir plusieurs données qui seraient
relatives à différentes choses que l'on a cherché à caractériser (e.g.,
la taille, la couleur, le poids, etc.), et qui seraient relatives
également à différents individus chez qui l'on aurait souhaité
caractériser ces choses. Afin de conduire les analyses qui permettraient
de répondre à la question posée, il convient alors de répertorier toutes
les données acquises dans un même document, et plus exactement dans un
même fichier, qui serait la base de données, telle que présentée dans le
Tableau~\ref{tbl-database}.
...
\caption{\label{tbl-database}Exemple de base de données}
...
La base de données prend donc la forme d'un tableau. Plusieurs principes
sont à respecter en général lors de la création d'une base de données.
Tout d'abord, les lignes de la base de données (qu'on appelle des
\textbf{observations}) doivent correspondre le cas échéant à des
individus bien identifiés. Ensuite, chaque colonne doit correspondre à
une variable. L'ensemble des données contenues dans une même ligne
correspond donc aux données relatives aux différentes variables qui
auraient été obtenues chez un même individu (e.g., la taille, le poids,
le sexe, etc.). Dans le cas d'études où l'on évaluerait une ou plusieurs
variables plusieurs fois chez un même individu (i.e., à différentes
moments, dans différentes conditions), il peut convenir de créer autant
de lignes que de fois où les variables auraient été évaluées. Par
exemple, le Tableau~\ref{tbl-databasePlusObs} représente une base de
données, certes très sommaire, qui contient des données d'individus dont
on aurait évalué le poids deux fois, avant et après un programme de
prise en charge. On remarque alors qu'il y a deux lignes par individu
qui correspondent aux deux temps d'évaluation. La taille, elle, n'a été
évaluée qu'une seule fois, en début de programme, mais pour éviter de
laisser des cellules vides, la valeur initiale de la taille a été
reproduite dans la seconde ligne.
...
\caption{\label{tbl-databasePlusObs}Organisation d'une base de données
...
En principe, les données de la base qui ont été obtenues selon la même
procédure d'acquisition représentent le même type de choses. Ces choses
sont appelées des \textbf{variables} car elles varient selon les
individus qui ont été étudiés et les conditions de mesure qui ont été
mises en oeuvre (dans le cas où il y en aurait plusieurs). Lorsque ces
choses ne sont pas censées varier, on parle de \textbf{constantes}. Une
base de données peut comporter des variables de types différents
(Tableau~\ref{tbl-typesVariables}).
...
\caption{\label{tbl-typesVariables}Les différents types de variables}
...
\subsection{\texorpdfstring{Sélectionner des lignes avec
\texttt{filter()}}{Sélectionner des lignes avec filter()}}\label{suxe9lectionner-des-lignes-avec-filter}

En fonction des besoins de l'analyse, on peut vouloir ne retenir que
certaines lignes du fichier de données. La fonction \texttt{filter()} du
package \texttt{\{dplyr\}} est faite pour réaliser ce filtrage.
Plusieurs opérateurs sont disponibles pour ne retenir que les lignes que
l'on veut (cf. Tableau~\ref{tbl-SymbolsFilter}).
...
\caption{\label{tbl-SymbolsFilter}Les opérateurs utilisables avec la
fonction filter()}
...
couplés à l'opérateur \texttt{\textbar{}} (OU) et à l'opérateur
\texttt{\&} (ET). Dans l'exemple ci-dessous, le code permet, à partir du
jeu de données \texttt{iris}, de ne garder que les lignes du jeu de
données qui contiennent les noms d'espèce \emph{setosa} OU
\emph{virginica}, ET en même temps qui affichent une longueur de sépale
...
\includegraphics[width=0.9\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-geomhistogram-1.pdf}
...
\includegraphics[width=1\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-geomhistogramVariedBinwidth2-1.pdf}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-geomboxplot-1.pdf}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-geomboxplotModified-1.pdf}

}

\caption{\label{fig-geomboxplotModified}Visualisation d'un outlier}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-geomboxplotOutlierId-1.pdf}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-raincloudUnivar-1.pdf}
...
\includegraphics[width=1\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-variousDistributions-1.pdf}
...
\includegraphics[width=1\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-meansSkewedVariables-1.pdf}
...
\includegraphics[width=1\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-mediansSkewedVariables-1.pdf}
...
\caption{\label{tbl-quantilesCompaMethods}Valeurs de quantiles obtenue
selon la méthode de calcul dans R}
...
On remarque que les résultats de la fonction \texttt{quantile()} sont
différents selon la configuration de l'argument \texttt{type}. Le choix
de la configuration est donc important. Pour comprendre comment R a
calculé les valeurs associées aux quantiles 0.25 et 0.75 dans le cadre
de la seconde méthode (i.e., avec \texttt{type} = 7), regardons le
tableau ci-dessous.
...
\caption{\label{tbl-quantilesContinuousMethod}Méthode de calcul des
quantiles}
...
Le tableau montre les données sur lesquelles R s'est appuyé pour
déterminer les valeurs des quantiles recherchés (i.e., les quantiles
0.25 et 0.75 pour Q1 et Q3, respectivement). Les données du tableau sont
bien celles relatives à notre variable \texttt{vec}, dont on peut
reconnaître les valeurs dans la colonne de droite du tableau. La colonne
``Quantile'' montre les fractions (ou portions) de la variable
\texttt{vec} associées aux valeurs de la variable compte tenu de leurs
rangs respectifs. Par exemple, la valeur 25.6, dont le rang est 6,
correspond au quantile 0.71 (approximativement). Cela veut dire que 71
\% des observations ont une valeur inférieure ou égale à 25.6. Il faut
savoir qu'il existe en réalité plusieurs manières de déterminer la
valeur du quantile que représente chaque valeur. Dans le cas présent, le
quantile représenté, que l'on va noter \emph{q}, a été déterminé selon
la formule suivante : \[q = (k - 1 ) / (N - 1)\] Dans le calcul
ci-dessus, \emph{k} désigne le rang de la valeur considérée, et \(N\)
désigne la taille de la variable étudiée (i.e., le nombre total de
valeurs). Comme on peut le voir dans le tableau ci-dessus, cette méthode
de calcul conduit nécessairement à attribuer le quantile 0 à la valeur
de rang 1, et la quantile 1 à la valeur de rang \(N\). Lorsque le nombre
de valeurs fait que les quantiles 0.25 et 0.75 n'existent pas, R réalise
une interpolation de la valeur correspondant au quantile recherché, cela
à partir des quantiles qui existent et qui encadrent le quantile
recherché, ainsi qu'à partir des valeurs correspondant à ces quantiles.
Dans le cas présent, il s'agit plus précisément d'une interpolation
linéaire. Voyons sur la Figure~\ref{fig-graphicContinuouQuantiles} en
quoi cela consiste.
...
\includegraphics[width=0.9\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-graphicContinuouQuantiles-1.pdf}
...
\includegraphics[width=1\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-skeweddistri-1.pdf}
...
\includegraphics[width=1\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-kurtosisdistri-1.pdf}
...
\begin{longtable}[]{@{}
  >{\raggedright\arraybackslash}p{(\columnwidth - 22\tabcolsep) * \real{0.1609}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 22\tabcolsep) * \real{0.1264}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 22\tabcolsep) * \real{0.1149}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 22\tabcolsep) * \real{0.1609}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 22\tabcolsep) * \real{0.0575}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 22\tabcolsep) * \real{0.0575}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 22\tabcolsep) * \real{0.0460}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 22\tabcolsep) * \real{0.0575}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 22\tabcolsep) * \real{0.0460}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 22\tabcolsep) * \real{0.0460}}
  >{\raggedleft\arraybackslash}p{(\columnwidth - 22\tabcolsep) * \real{0.0575}}
  >{\raggedright\arraybackslash}p{(\columnwidth - 22\tabcolsep) * \real{0.0690}}@{}}
\toprule\noalign{}
\begin{minipage}[b]{\linewidth}\raggedright
skim\_variable
\end{minipage} & \begin{minipage}[b]{\linewidth}\raggedright
...
\includegraphics[width=0.9\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-ObservationsWithinSdMutiplesGaussianDistri-1.pdf}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-MeanAboveLedianAsym-1.pdf}
...
\includegraphics[width=0.8\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-geombar-1.pdf}
...
\includegraphics[width=1\textwidth,height=\textheight]{univariate_analyses_files/figure-pdf/fig-geombarPropEnhancedGraph-1.pdf}
...
\includegraphics[width=1\textwidth,height=\textheight]{bivariate_analyses_files/figure-pdf/fig-relationshipsIllustrations-1.pdf}
...
\includegraphics{bivariate_analyses_files/figure-pdf/fig-scatterplot-1.pdf}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{bivariate_analyses_files/figure-pdf/fig-scatterplotLine-1.pdf}
...
\caption{\label{tbl-tablecov}Illustration de calculs de covariance}
...
\multicolumn{1}{>{\cellcolor[HTML]{CFCFCF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{\textbf{X1}}}}} & \multicolumn{1}{>{\cellcolor[HTML]{CFCFCF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{\textbf{Y1}}}}} & \multicolumn{1}{>{\cellcolor[HTML]{CFCFCF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{\textbf{Y2}}}}} & \multicolumn{1}{>{\cellcolor[HTML]{CFCFCF}\centering}m{\dimexpr 0.51in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{\textbf{Y3}}}}} & \multicolumn{1}{>{\cellcolor[HTML]{CFCFCF}\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{\textbf{X1Y1}}}}} & \multicolumn{1}{>{\cellcolor[HTML]{CFCFCF}\centering}m{\dimexpr 1.06in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{\textbf{X1Y2}}}}} & \multicolumn{1}{>{\cellcolor[HTML]{CFCFCF}\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{\textbf{X1Y3}}}}} \\

\noalign{\global\arrayrulewidth 0pt}\arrayrulecolor[HTML]{000000}

\hhline{>{\arrayrulecolor[HTML]{000000}\global\arrayrulewidth=1.5pt}->{\arrayrulecolor[HTML]{000000}\global\arrayrulewidth=1.5pt}->{\arrayrulecolor[HTML]{000000}\global\arrayrulewidth=1.5pt}->{\arrayrulecolor[HTML]{000000}\global\arrayrulewidth=1.5pt}->{\arrayrulecolor[HTML]{000000}\global\arrayrulewidth=1.5pt}->{\arrayrulecolor[HTML]{000000}\global\arrayrulewidth=1.5pt}->{\arrayrulecolor[HTML]{000000}\global\arrayrulewidth=1.5pt}-}\endfirsthead 
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{0}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{0}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{0}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.51in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{0}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{36}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.06in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{40.285714}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{-36}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{2}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{2}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{1}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.51in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{-2}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{16}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 1.06in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{22.857143}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{-16}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{4}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{4}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{15}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.51in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{-4}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{4}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.06in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{-16.571429}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{-4}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{6}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{6}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{5}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.51in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{-6}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{0}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 1.06in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{0.000000}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{0}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{8}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{8}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{11}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.51in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{-8}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{4}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.06in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{8.571429}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{-4}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{10}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{10}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{3}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.51in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{-10}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{16}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 1.06in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{-14.857143}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{-16}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{12}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{12}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.47in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{12}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.51in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{-12}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{36}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.06in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{31.714286}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.66in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{-36}}}} \\
...
\includegraphics[width=1\textwidth,height=\textheight]{bivariate_analyses_files/figure-pdf/fig-graphCov-1.pdf}
...
Pour visualiser le lien que l'on peut faire entre la forme du nuage de
points et la valeur du coefficient de corrélation de Pearson que l'on
peut obtenir, la page web proposée par Kristoffer Magnusson
(https://rpsychologist.com/correlation) peut être particulièrement
intéressante. Cette page web donne la possibilité de faire varier
manuellement la valeur du coefficient de corrélation de Pearson pour
ensuite voir un nuage de points type correspondant à cette valeur.
Faites un essai !

À noter que la valeur du coefficient de corrélation de Pearson est
dépendante du degré de variabilité des valeurs autour des tendances
centrales des variables concernées. (Halperin, 1986). En ce sens, si
dans deux études différentes les variables étudiées n'ont pas les mêmes
niveaux de variabilité, les valeurs des coefficients de corrélation de
Pearson ne seront pas vraiment comparables, en sachant que c'est l'étude
qui présentera l'échantillon avec la plus grande variabilité qui aura
plus de chances de présenter une valeur de coefficient de corrélation de
Pearson plus élevée.
...
\includegraphics[width=0.9\textwidth,height=\textheight]{bivariate_analyses_files/figure-pdf/fig-simpsonquant-1.pdf}

}

\subcaption{\label{fig-simpsonquant}Données obtenues grâce à la fonction
simulate\_simpson() du package \{bayestestR\}}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{bivariate_analyses_files/figure-pdf/fig-spuriousCorrelationBis-1.pdf}
...
\caption{\label{tbl-tableranks}Illustration de la notion de rang pour
...
\multicolumn{1}{>{\cellcolor[HTML]{CFCFCF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{\textbf{hp\ (valeur)}}}}} & \multicolumn{1}{>{\cellcolor[HTML]{CFCFCF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{\textbf{hp\ (rang)}}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{52}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{1.0}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{62}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{2.0}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{65}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{3.0}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{66}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{4.5}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{66}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{4.5}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{91}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{6.0}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{93}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{7.0}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{95}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{8.0}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{97}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{9.0}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{105}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{10.0}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{109}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{11.0}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{110}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{13.0}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{110}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{13.0}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{113}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{15.0}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{123}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{16.5}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{123}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{16.5}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{150}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{18.5}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{150}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{18.5}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{175}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{21.0}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{175}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{21.0}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{180}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{24.0}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{180}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{24.0}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{205}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{26.0}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{215}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{27.0}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{230}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{28.0}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{245}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{29.5}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{245}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{29.5}}}} \\
...
\multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{264}}}} & \multicolumn{1}{>{\cellcolor[HTML]{EFEFEF}\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{31.0}}}} \\
...
\multicolumn{1}{>{\centering}m{\dimexpr 1.07in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{335}}}} & \multicolumn{1}{>{\centering}m{\dimexpr 0.95in+0\tabcolsep}}{\textcolor[HTML]{000000}{\fontsize{11}{11}\selectfont{\global\setmainfont{Arial}{32.0}}}} \\
...
Le fait d'étudier l'existence d'une relation linéaire entre les rangs et
non plus entre les valeurs de deux variables permet de s'affranchir de
l'influence possible de valeurs très extrêmes, dans l'une et/ou l'autre
variable, sur le calcul final de la corrélation. Pour déterminer alors
la valeur de la corrélation, une manière de procéder est d'appliquer la
méthode de calcul du coefficient de corrélation de Pearson en utilisant
non plus les valeurs des variables, mais les rangs correspondants. Cette
methode, c'est celle du calcul du coefficient de corrélation de Spearman
(\emph{rho}). Si l'on suit \emph{stricto sensu} cette définition, nous
pourrions alors utiliser le code suivant pour avoir le coefficient de
...
\includegraphics[width=0.9\textwidth,height=\textheight]{bivariate_analyses_files/figure-pdf/fig-graphSpearman-1.pdf}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{bivariate_analyses_files/figure-pdf/fig-ranksvvaluesspearman-1.pdf}
...
\includegraphics[width=1\textwidth,height=\textheight]{bivariate_analyses_files/figure-pdf/fig-groupedBarsFigure-1.pdf}
...
\includegraphics[width=1\textwidth,height=\textheight]{bivariate_analyses_files/figure-pdf/fig-groupedBarsUpgraded-1.pdf}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{bivariate_analyses_files/figure-pdf/fig-stackedBarsBiv-1.pdf}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{bivariate_analyses_files/figure-pdf/fig-PhiContTables-1.pdf}
...
\NormalTok{effectsize}\SpecialCharTok{::}\FunctionTok{cramers\_v}\NormalTok{(tab, }\AttributeTok{adjust =} \ConstantTok{FALSE}\NormalTok{) }\CommentTok{\# Mettre TRUE pour appliquer une correction en cas de petits échantillons}
\end{Highlighting}
\end{Shaded}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{two_group_comparison_files/figure-pdf/fig-raindcloudplotsBivUnpaired1-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{gd }\OtherTok{\textless{}{-}}
  \FunctionTok{glass\_delta}\NormalTok{(}
\NormalTok{    Sepal.Length }\SpecialCharTok{\textasciitilde{}}\NormalTok{ Species, }
    \AttributeTok{data =}\NormalTok{ iris\_two\_species}
\NormalTok{    )}
\NormalTok{gd}
...
Notons quand même que l'utilisation du \(\Delta\) de Glass semble peu
courante dans les études cherchant à tester l'effet d'un programme.
L'une des difficultés liées à son utilisation est que cela suggère que
les groupes à comparer soient identiques en pré-programme pour la
variable étudiée. En général, des procédures un peu plus sophistiquées
sont utilisées pour déterminer l'effet d'un programme, en particulier
des procédures qui permettent de prendre en compte justement les
différences qui peuvent exister entre les groupes en pré-programme.

\subparagraph{Qualification de l'importance de la différence de moyennes
standardisée}\label{qualification-de-limportance-de-la-diffuxe9rence-de-moyennes-standardisuxe9e}

Une fois que l'on a calculé une taille d'effet, il est toujours
intéressant d'essayer de formuler un jugement sur l'importance,
l'ampleur de l'effet. En ce sens, des valeurs seuils ont été proposées
dans la littérature pour les différences de moyennes standardisées
(Cohen, 1988; in Lakens (2013)). Ces valeurs, valables pour interpréter
des tailles d'effet dans le cadre d'une étude de type
\emph{between-subject design}, sont montrées dans le
Tableau~\ref{tbl-effectsizedtable} ci-dessous.
...
\caption{\label{tbl-effectsizedtable}Proposition de termes pour
qualifier l'importance d'une différence de moyennes standardisée}
...
Pour obtenir une interprétation automatique en même temps que la valeur
de différence de moyennes standardisée, on peut utiliser les fonctions
\texttt{interpret\_cohens\_d()}, \texttt{interpret\_hedges\_g()} et
\texttt{interpret\_glass\_delta()} du package \{effectsize\} :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{interpret\_cohens\_d}\NormalTok{(ds)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{interpret\_hedges\_g}\NormalTok{(gs)}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Calcul des rangs relatifs aux observations des deux modalités confondues}
\NormalTok{ranks }\OtherTok{\textless{}{-}} \FunctionTok{rank}\NormalTok{(iris\_two\_species}\SpecialCharTok{$}\NormalTok{Sepal.Length)}

\CommentTok{\# Obtention du nombre d\textquotesingle{}observations pour setosa et versicolor}
\NormalTok{n\_setosa }\OtherTok{\textless{}{-}} \FunctionTok{nrow}\NormalTok{(iris\_two\_species }\SpecialCharTok{|\textgreater{}} \FunctionTok{filter}\NormalTok{(Species }\SpecialCharTok{==} \StringTok{"setosa"}\NormalTok{))}
\NormalTok{n\_versicolor }\OtherTok{\textless{}{-}} \FunctionTok{nrow}\NormalTok{(iris\_two\_species }\SpecialCharTok{|\textgreater{}} \FunctionTok{filter}\NormalTok{(Species }\SpecialCharTok{==} \StringTok{"versicolor"}\NormalTok{))}

\CommentTok{\# Calcul de la somme des rangs normalisée pour l\textquotesingle{}espèce \textasciigrave{}setosa\textasciigrave{}}
\NormalTok{sum\_rks\_setosa }\OtherTok{\textless{}{-}}
\NormalTok{  (}\FunctionTok{sum}\NormalTok{(ranks[}\FunctionTok{seq\_along}\NormalTok{(iris\_two\_species[iris\_two\_species}\SpecialCharTok{$}\NormalTok{Species }\SpecialCharTok{==} \StringTok{"setosa"}\NormalTok{, }\StringTok{"Sepal.Length"}\NormalTok{])]) }\SpecialCharTok{{-}}\NormalTok{ n\_setosa }\SpecialCharTok{*}\NormalTok{ (n\_setosa }\SpecialCharTok{+} \DecValTok{1}\NormalTok{) }\SpecialCharTok{/}
     \DecValTok{2}\NormalTok{) }\SpecialCharTok{/}\NormalTok{ (n\_setosa }\SpecialCharTok{*}\NormalTok{ n\_versicolor)}

\CommentTok{\# Calcul de la somme des rangs normalisée pour l\textquotesingle{}espèce \textasciigrave{}versicolor\textasciigrave{}}
\NormalTok{sum\_rks\_versicolor }\OtherTok{\textless{}{-}}
\NormalTok{  (}\FunctionTok{sum}\NormalTok{(ranks[}\SpecialCharTok{{-}}\FunctionTok{seq\_along}\NormalTok{(iris\_two\_species[iris\_two\_species}\SpecialCharTok{$}\NormalTok{Species }\SpecialCharTok{==} \StringTok{"versicolor"}\NormalTok{, }\StringTok{"Sepal.Length"}\NormalTok{])]) }\SpecialCharTok{{-}}\NormalTok{ n\_setosa }\SpecialCharTok{*}\NormalTok{ (n\_setosa }\SpecialCharTok{+} \DecValTok{1}\NormalTok{) }\SpecialCharTok{/}
     \DecValTok{2}\NormalTok{) }\SpecialCharTok{/}\NormalTok{ (n\_setosa }\SpecialCharTok{*}\NormalTok{ n\_versicolor)}

\CommentTok{\# Obtention de la corrélation bisériale}
\NormalTok{janitor}\SpecialCharTok{::}\FunctionTok{round\_half\_up}\NormalTok{(sum\_rks\_setosa }\SpecialCharTok{{-}}\NormalTok{ sum\_rks\_versicolor, }\DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
Le résultat indique que les observations liées à la seconde modalité de
la variable \texttt{Species}, qui est \texttt{versicolor}, ont en
général des valeurs supérieures à celles relatives à la modalité
\texttt{setosa}.

La grandeur de la corrélation bisériale des rangs peut être caractérisée
à l'aide de la classification de Funder \& Ozer (2019) proposée par
défaut dans le package \{effectsize\}
(Tableau~\ref{tbl-effectsizecorrb}) :
...
\caption{\label{tbl-effectsizecorrb}Proposition de termes pour qualifier
l'importance d'une corrélation bisériale des rangs}
...
Pour obtenir une interprétation automatique en même temps que la valeur
de corrélation, on peut utiliser la fonction \texttt{interpret\_rb()} du
package \{effectsize\} :

\begin{Shaded}
\begin{Highlighting}[]
\FunctionTok{interpret\_rank\_biserial}\NormalTok{(rb)}
\end{Highlighting}
\end{Shaded}
...
\paragraph{Langage commun}\label{langage-commun-1}

Pour comparer les groupes en utilisant la probabilité de supériorité, on
peut utiliser la fonction \texttt{p\_superiority()} du package
...
\includegraphics[width=0.9\textwidth,height=\textheight]{two_group_comparison_files/figure-pdf/fig-raindcloudplotsBivPaired1-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Calcul des différences entre les modalités}
\NormalTok{diffs }\OtherTok{\textless{}{-}}\NormalTok{ Blink2[Blink2}\SpecialCharTok{$}\NormalTok{Condition }\SpecialCharTok{==} \StringTok{"Straight"}\NormalTok{, }\StringTok{"Blink\_rate"}\NormalTok{] }\SpecialCharTok{{-}}\NormalTok{ Blink2[Blink2}\SpecialCharTok{$}\NormalTok{Condition }\SpecialCharTok{==} \StringTok{"Oscillating"}\NormalTok{, }\StringTok{"Blink\_rate"}\NormalTok{]}

\CommentTok{\# Calcul des rangs signés relatifs aux observations des deux modalités confondues}
\NormalTok{ranks }\OtherTok{\textless{}{-}}\NormalTok{ datawizard}\SpecialCharTok{::}\FunctionTok{ranktransform}\NormalTok{(diffs, }\AttributeTok{sign =} \ConstantTok{TRUE}\NormalTok{) }\SpecialCharTok{|\textgreater{}} \FunctionTok{pull}\NormalTok{(Blink\_rate)}

\CommentTok{\# Obtention du nombre d\textquotesingle{}individus}
\NormalTok{n }\OtherTok{\textless{}{-}} \FunctionTok{nrow}\NormalTok{(Blink2 }\SpecialCharTok{|\textgreater{}} \FunctionTok{filter}\NormalTok{(Condition }\SpecialCharTok{==} \StringTok{"Straight"}\NormalTok{))}

\CommentTok{\# Calcul de la somme des rangs positifs normalisée}
\NormalTok{sum\_rks\_pos }\OtherTok{\textless{}{-}} \FunctionTok{sum}\NormalTok{(ranks[ranks }\SpecialCharTok{\textgreater{}} \DecValTok{0}\NormalTok{]) }\SpecialCharTok{/}\NormalTok{ (n }\SpecialCharTok{*}\NormalTok{ (n }\SpecialCharTok{+} \DecValTok{1}\NormalTok{) }\SpecialCharTok{/} \DecValTok{2}\NormalTok{)}

\CommentTok{\# Calcul de la somme des rangs négatifs normalisée}
\NormalTok{sum\_rks\_neg }\OtherTok{\textless{}{-}} \SpecialCharTok{{-}}\FunctionTok{sum}\NormalTok{(ranks[ranks }\SpecialCharTok{\textless{}} \DecValTok{0}\NormalTok{]) }\SpecialCharTok{/}\NormalTok{ (n }\SpecialCharTok{*}\NormalTok{ (n }\SpecialCharTok{+} \DecValTok{1}\NormalTok{) }\SpecialCharTok{/} \DecValTok{2}\NormalTok{)}

\CommentTok{\# Obtention de la corrélation bisériale}
\NormalTok{janitor}\SpecialCharTok{::}\FunctionTok{round\_half\_up}\NormalTok{(sum\_rks\_pos }\SpecialCharTok{{-}}\NormalTok{ sum\_rks\_neg, }\DecValTok{2}\NormalTok{)}
\end{Highlighting}
\end{Shaded}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{two_group_comparison_files/figure-pdf/fig-taballUCBA-1.pdf}
...
\includegraphics[width=1\textwidth,height=\textheight]{two_group_comparison_files/figure-pdf/fig-tabdepUCBA-1.pdf}
...
\includegraphics[width=1\textwidth,height=\textheight]{two_group_comparison_files/figure-pdf/fig-distriUCBA-1.pdf}
...
\includegraphics[width=0.7\textwidth,height=\textheight]{two_group_comparison_files/figure-pdf/fig-RiskContTables-1.pdf}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{\# Réarangement des niveaux de sexe}
\NormalTok{res}\SpecialCharTok{$}\NormalTok{sex }\OtherTok{\textless{}{-}} \FunctionTok{factor}\NormalTok{(res}\SpecialCharTok{$}\NormalTok{sex, }\AttributeTok{levels =} \FunctionTok{c}\NormalTok{(}\StringTok{"Hommes"}\NormalTok{, }\StringTok{"Femmes"}\NormalTok{))}

\CommentTok{\# Création du tableau de contingence}
\NormalTok{tab }\OtherTok{\textless{}{-}} \FunctionTok{with}\NormalTok{(res, }\FunctionTok{table}\NormalTok{(result, sex))}
\NormalTok{tab}
\end{Highlighting}
\end{Shaded}
...
\includegraphics[width=0.7\textwidth,height=\textheight]{regressions_files/figure-pdf/fig-graphsee-1.pdf}
...
\includegraphics[width=0.8\textwidth,height=\textheight]{regressions_files/figure-pdf/fig-graphLinStats-1.pdf}
...
\includegraphics[width=0.7\textwidth,height=\textheight]{regressions_files/figure-pdf/fig-anscombeQuartetGraph-1.pdf}
...
\includegraphics[width=0.8\textwidth,height=\textheight]{statistical_theory_files/figure-pdf/fig-distriBinom-1.pdf}
...
\includegraphics[width=0.9\textwidth,height=\textheight]{statistical_theory_files/figure-pdf/fig-distiNorm-1.pdf}
...
\includegraphics[width=1\textwidth,height=\textheight]{statistical_theory_files/figure-pdf/fig-VariousQuantLaws-1.pdf}
...
\includegraphics[width=1\textwidth,height=\textheight]{statistical_theory_files/figure-pdf/fig-GreatNumLaw-1.pdf}
...
\includegraphics[width=0.8\textwidth,height=\textheight]{statistical_theory_files/figure-pdf/fig-CentralLimTheo-1.pdf}
...
\includegraphics[width=0.8\textwidth,height=\textheight]{statistical_theory_files/figure-pdf/fig-CentralLimTheo2-1.pdf}
...
\includegraphics[width=0.8\textwidth,height=\textheight]{categorical_var_tests_files/figure-pdf/fig-FreqCards-1.pdf}
...
\includegraphics[width=0.8\textwidth,height=\textheight]{categorical_var_tests_files/figure-pdf/fig-GOFDistri-1.pdf}
...
\bibitem[\citeproctext]{ref-allenRaincloudPlotsMultiplatform2019}
...
\bibitem[\citeproctext]{ref-anscombeGraphsStatisticalAnalysis1973}
Anscombe, F. J. (1973). Graphs in Statistical Analysis. \emph{Am Stat},
\emph{27}(1), 17‑21.

\bibitem[\citeproctext]{ref-bickelSexBiasGraduate1975}
...
\bibitem[\citeproctext]{ref-chatellierMoyenneMedianeLeurs2003}
...
\bibitem[\citeproctext]{ref-dartCommentDecrireDistribution2003}
...
\bibitem[\citeproctext]{ref-gonzalesMeasuresCentralTendency2001}
...
\bibitem[\citeproctext]{ref-grenierQuelleEstBonne2007}
Grenier, E. (2007). Quelle Est La « Bonne » Formule de l'écart-Type ?
\emph{Revue MODULAD}, \emph{37}, 102‑105.

\bibitem[\citeproctext]{ref-halperinSpuriousCorrelationsCauses1986}
...
\bibitem[\citeproctext]{ref-hopkinsProgressiveStatisticsStudies2009}
...
\bibitem[\citeproctext]{ref-janeEffectSizesConfidence2023}
...
\bibitem[\citeproctext]{ref-joanesComparingMeasuresSample1998}
...
\bibitem[\citeproctext]{ref-kelleyEffectsNonnormalDistributions2005}
...
\bibitem[\citeproctext]{ref-labreucheDifferentsTypesVariables2010}
...
\bibitem[\citeproctext]{ref-lakensCalculatingReportingEffect2013}
Lakens, D. (2013). Calculating and Reporting Effect Sizes to Facilitate
Cumulative Science: A Practical Primer for t-Tests and {ANOVAs}.
\emph{Front. Psychol.}, \emph{4}.
\url{https://doi.org/10.3389/fpsyg.2013.00863}

\bibitem[\citeproctext]{ref-liEffectSizeMeasures2016}
...
\bibitem[\citeproctext]{ref-navarroLearningStatistics2018}
Navarro, D. (2018). \emph{Learning Statistics with {R}}. UNSW
Computational Cognitive Science.

\bibitem[\citeproctext]{ref-rousseletReactionTimesOther2020}
...
\bibitem[\citeproctext]{ref-weissgerberBarLineGraphs2015}
...
\bibitem[\citeproctext]{ref-wickhamDataScience2017}
Wickham, H., \& Grolemund, G. (2017). \emph{R for {Data Science}}.
O'Reilly.

\bibitem[\citeproctext]{ref-wilkeFundamentalsDataVisualization2018}
