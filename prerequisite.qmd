...
* Aller sur la page web [https://cran.rstudio.com](https://cran.rstudio.com).
...
* Aller sur la page web [https://posit.co/download/rstudio-desktop/](https://posit.co/download/rstudio-desktop/).
...
* **Sur WINDOWS :** Exécuter le fichier s'il est proposé de le faire. Si le clic entraîne automatiquement le téléchargement du fichier, retrouver alors ce fichier sur le PC (une fois son enregistrement terminé), puis exécuter le fichier et suivre la procédure d’installation par défaut. Une fois le logiciel installé, retrouver le fichier d’exécution du logiciel sur le PC (chemin d'accès possible : **Ordinateur > Windows (C:) > Programmes > RStudio > rstudio.exe**). Créer un raccourci pour le fichier **rstudio.exe** et le mettre sur le bureau du PC ou l'épingler dans la barre de tâches. Double-cliquer sur l’icône RStudio afin de vérifier si l’ouverture s’effectue correctement. 
...
```{r calculations_with_vector_objects}
vec1 <- c(0, 2, 4, 6, 8)
vec2 <- c(1, 4, 5, 9, 0)
vec1 * 10
vec1 * vec2
...
```{r calculations_with_data_frame_objects}
...
En revanche, si nous avions inversé l'ordre d'écriture des variables dans la fonction sans préciser les noms des arguments (cf. code ci-dessous), nous aurions eu un résultat différent du graphique précédent (cf. @fig-plot_function5), où les variables en x et en y ont été inversées par rapport au graphique précédent).

```{r plot_function5}
#| fig-align: "center"
#| fig-cap: "Graphique obtenu avec un paramétrage implicite de la fonction plot()"
#| out-width: '90%'
#| label: fig-plot_function5
