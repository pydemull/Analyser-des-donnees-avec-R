...
avec $rank(X_{i})$ et $rank(Y_{i})$ les rangs des observations des variables $X$ et $Y$ toutes observations confondues, et $N_{X}$ et $N_{Y}$ les nombres d'observations pour les variables $X$ et $Y$. Cette corrélation peut être obtenue manuellement dans R de la manière suivante : 

```{r}
# Exemple avec le jeu de données `iris_two_species` crée plus haut
# On cherche à comparer les espèces `setosa`et `versicolor` pour la 
...
On note, dans le code ci-dessus, que l'argument `parametric` est configuré avec la valeur `FALSE`. De cette manière, la probabilité de supériorité reste calculée à partir des rangs à partir de la corrélation bisériale des rangs tel que : $Pr(Superiority) = (r_{rb} + 1)/2$. Ce calcul serait le plus précis (le moins biaisé) sur un ensemble de scenarii de violation des conditions de normalité et d'hétérogénéité de variance [@liEffectSizeMeasures2016].
...
Le $d_{z}$ peut heureusement être obtenu à nouveau avec la fonction `rm_d()` du package `{effectsize}`. Pour illustrer cela, nous allons utiliser le jeu de données `Blink2` crée plus haut pour l'exemple de graphique :
...
avec $SignedRank(X_{i} - Y_{i}))$ les rangs signés associés aux observations des modalités $X$ et $Y$ toutes observations confondues, et $N$ les nombres d'individus étudiés. Cette corrélation peut être obtenue manuellement dans R de la manière suivante : 
...
Le résultat indique que les observations liée à la deuxième modalité de la variable `Condition`, qui est `Oscillating`, ont systématiquement des valeurs inférieures à celles relatives à la modalité `Straight`.
...
* Les indices de différence de moyennes standardisée non biaisés sont les $g_{s}$ et $g_{av}$ de Hedges en cas de groupes indépendants et les $g_{z}$ et $g_{av}$ de Hedges en cas de groupes dépendants et peuvent être calculés avec la fonction `effectsize::hedges_g()`.
