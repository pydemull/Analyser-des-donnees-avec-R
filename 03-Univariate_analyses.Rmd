...
```{r geomhistogram, fig.cap = "Exemple d'histogramme"}
...
```{r geomhistogramVariedBinwidth2, echo = FALSE,  fig.cap="Différentes largeurs d'intervalles pour un histogramme"}
...
```{r geomboxplot, fig.height=3, fig.cap="Exemple de boîte à moustaches"}
ggplot(data = iris, aes(x = Sepal.Length)) + 
  geom_boxplot() +
...
```{r geomboxplotModified, fig.height=3, fig.cap="Visualisation d'un outlier"}
...
```{r geomboxplotOutlierId, fig.height=3, fig.cap="Identification d'un outlier"}
...
# Le nombre entre crochets désigne la position de l'observation dans la variable.
```
...
```{r gdistriExample, message = FALSE, out.width = '50%', fig.cap="Visualisation de la distribution avec la fonction `plot_distri()`"}
...
```{r variousDistributions, echo = FALSE, message = FALSE, fig.cap="Différents types de distributions"}
set.seed(123)
...
```{r meansSkewedVariables, echo = FALSE, message = FALSE, warning = FALSE, fig.cap="Moyenne d'une variable présentant une distribution asymétrique"}
# Get legend function
...
```{r mediansSkewedVariables, echo = FALSE, message = FALSE, warning = FALSE, fig.cap = "Médiane d'une variable présentant une distribution asymétrique"}
# Graphique pour la distribution gaussienne
set.seed(123)
gauss <- tibble(x = rnorm(n = 1000, mean = 0, sd = 1))
mean_gauss <- mean(gauss$x)
...
```{r graphicContinuouQuantiles, echo = FALSE, fig.cap="Calcul des quartiles avec une variable quantitative continue"}
...
```{r skeweddistri, echo = FALSE, message = FALSE, warning = FALSE, fig.cap="Valeur du Skewness selon la forme de la distribution"}
set.seed(123)
...
```{r kurtosisdistri, echo = FALSE, message = FALSE, warning = FALSE,  fig.cap="Valeur du Kurtosis selon la forme de la distribution"}
...
```{r ObservationsWithinSdMutiplesGaussianDistri, echo = FALSE, warning = FALSE, message = FALSE, fig.cap="Proportions des observations incluses dans différents intervalles liés à des multiples de l'écart-type"}
set.seed(123)
...
```{r MeanAboveLedianAsym, echo = FALSE, warning = FALSE, message = FALSE, fig.cap = "Moyenne et médiane dans le cadre d'une distribution asymétrique"}
...
```{r geombar, fig.cap="Exemple de diagramme en barres"}
...
```{r geombarEnhanced, message = FALSE, fig.cap="Diagramme en barres obtenu avec une seconde méthode d'écriture du code"}
diamonds %>% 
  group_by(color) %>%
  summarize(count = n())
