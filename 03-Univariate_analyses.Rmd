...
ggplot(data = iris, aes(x = "", y = Sepal.Length)) + 
  ggdist::stat_halfeye(
    ## Ajuste le lissage de la courbe de densité
    adjust = .5, 
    ## Ajuste la hauteur de l'aire située sous la courbe de densité
    width = .6, 
    .width = 0, 
    point_colour = NA
    ) + 
...
require(gghalves)
 ...
 ggplot(data = data, aes(x = {{x}}, y = {{y}})) + 
  ggdist::stat_halfeye(
    ## Ajuste le lissage de la courbe de densité
    adjust = .5, 
    ## Ajuste l'aire située sous la courbe de densité
    width = .6, 
    .width = 0, 
    point_colour = NA
    ) + 
...
Lorsqu'on regarde le code ci-dessus qui a permis de créer le *raincould plot*, on remarque l'arrivée de deux nouveaux packages : `ggdist` et `gghalves`. Ces packages doivent être installés et chargés avant de pouvoir être utilisés. Le package `ggdist` est utilisé avec la fonction `stat_halfeye()`, qui sert à montrer sur la figure ci-dessus l'aire grise sous la courbe de densité. Plus cette aire est grande au niveau d'un intervalle de valeurs donné, plus il est probable de rencontrer les valeurs concernées lorsqu'on prend au hasard des observations du jeu de données. Plusieurs arguments peuvent en réalité servir à calculer la courbe de densité et finalement modifier sa forme et celle de l'aire qu'il y a en-dessous. Parmi eux, l'argument `adjust` de la fonction `stat_halfeye()` sert en quelque sorte à ajuster le lissage de la courbe de densité : plus sa valeur sera élevée, plus la courbe sera lissée. 
...
A <- 
  ...
  ggplot(aes(x = color, y = n)) +
     geom_bar(stat = "identity") +
     geom_text(aes(label = n, x = color, y = n), 
               size = 2, nudge_y = 700) +
     ggtitle("A")
