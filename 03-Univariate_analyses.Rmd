...
Lorsqu'on regarde le code ci-dessus qui a permis de créer le *raincould plot*, on remarque l'arrivée de deux nouveaux packages : `ggdist` et `gghalves`. Ces packages doivent être installés et chargés avant de pouvoir être utilisés. La package `ggdist` est utilisé avec la fonction `stat_halfeye()`, qui sert à montrer l'aire sous la courbe de densité en gris sur la figure ci-dessus. Plus l'aire est grande au niveau d'un intervalle de valeurs de données, plus il est probable de rencontrer les valeurs concernées lorsqu'on prend au hasard des observations du jeu de données. Plusieurs arguments peuvent en réalité servir à calculer cette aire et finalement modifier sa forme. L'argument `adjust` de la fonction `stat_halfeye()` sert en quelque sorte à ajuster le lissage de l'aire de densité : plus sa valeur sera élevée, plus l'aire sera lissée. 
