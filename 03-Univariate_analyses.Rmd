...
L'utilisation de la fonction `plot_distri()` est illustrée ci-dessous (pour rappel, il est important d'activer/créer la fonction avant de pouvoir l'utiliser) :

```{r gdistriExample, message = FALSE, fig.align ="center", fig.cap="Visualisation de la distribution avec la fonction `plot_distri()`"}
...
gauss_g <- plot_distri(x = gauss)
skew_right_g <- plot_distri(x = skew_right)
lepto_g <- plot_distri(x = lepto) +  coord_cartesian(xlim = c(-5, 5))
plati_g <- plot_distri(x = plati)
