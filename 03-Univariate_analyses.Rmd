...
```{r geom_histogram with varied binwidth_2, echo = FALSE}
...
Une fois cette procédure réalisée, le reste du code, et notamment la fonction `geom_text()`, a permis d'ajouter des éléments textuels au graphique, en l'occurence en s'appuyant sur la variable id_outlier, tel que configuré avec l'argument `aes()` de la fonction `geom_text()`. 
...
```{r vizualising outliers with ggstatplot, warning = FALSE, message = FALSE}
library(ggstatsplot) 
iris %>%
  mutate(id = as.factor(rep(1:50, times = 3))) %>%
...
g_distri <- function(x = NULL, binwidth = NULL) {
  ...
  g2 <- data %>% 
    ...
    g3 <- data %>% 
    ggplot(aes(x = x)) +
    geom_point(aes(x = x, y = ""), shape = 1, size = 3) +
      ylab("") +
    coord_cartesian(xlim = c(min(x, na.rm = TRUE), max(x, na.rm = TRUE))) +
    theme(axis.text.y = element_blank(),
          axis.ticks.y = element_blank())
...
```{r g_distri example, message = FALSE}
...
```{r various distributions, echo = FALSE, message = FALSE}
set.seed(123)
...
```{r means with skewed variables, echo = FALSE, message = FALSE, warning = FALSE}
# Get legend function
...
mean_gauss_g <- ggplot(data = gauss) +
...
mean_skew_g <- ggplot(data = skew_right) +
  geom_histogram(aes(x = x), fill = "white", color = "black") +
...
```{r medians with skewed variables, echo = FALSE, message = FALSE, warning = FALSE}
# Graphique pour la distribution gaussienne
set.seed(123)
gauss <- tibble(x = rnorm(n = 1000, mean = 0, sd = 1))
mean_gauss <- mean(gauss$x)
median_gauss <- median(gauss$x)
median_gauss_g <- ggplot(data = gauss) +
  geom_histogram(aes(x = x), fill = "white", color = "black") +
...
median_skew_g <- ggplot(data = skew_right) +
  geom_histogram(aes(x = x), fill = "white", color = "black") +
...
```{r quantiles compa methods, echo = FALSE}
...
```{r quantiles continuous method, echo = FALSE}
...
```{r graphic for continuous quantiles, echo = FALSE}
...
```{r skewed distri, echo = FALSE, message = FALSE, warning = FALSE}
set.seed(123)
...
g_chis4_neg <- ggplot(data = chis_4_neg) +
  geom_histogram(aes(x = x), fill = "white", color = "black", bins = 20) +
  xlab("x") +
  theme(legend.position="bottom", legend.text = element_text(size = 13)) +
  labs(color = "")+
...
g_chis1_neg <- ggplot(data = chis_1_neg) +
  geom_histogram(aes(x = x), fill = "white", color = "black", bins = 20) +
  xlab("x") +
  theme(legend.position="bottom", legend.text = element_text(size = 13)) +
  labs(color = "") +
...
```{r kurtosis distri, echo = FALSE, message = FALSE, warning = FALSE}
...
La fonction `describe()` peut être aussi utilisée sur un jeu de données entier. (Attention, les résumés numériques fournis pour les variables qualitatives n'auront pas de sens ; les variables qualitatives détectées sont indiquées avec un astérique dans le tableau de résultats.) 
...
```{r observations within sd mutiples with gaussian distributions, echo = FALSE, warning = FALSE, message = FALSE}
set.seed(123)
...
```{r mean above median aysm, echo = FALSE, warning = FALSE, message = FALSE}
...
```{r geom_bar enhanced, message = FALSE}
diamonds %>% 
  group_by(color) %>%
  summarize(count = n())
...
# Diagrammes en barres empilées montrant les effectifs
g1 <- diamonds %>% 
      group_by(color) %>%
      summarize(count = n()) %>%
...
```{r geom_bar prop enhanced graph, echo = FALSE, message = FALSE}
(g1 | g2) / (g3 | g4)
```
...
* Dans R, les graphiques peuvent être réalisés, en autres, à l'aide du package `ggplot2` et des fonctions associées.
...
* Les variables qualitatives nominales devraient être visualsiées avec une organisation des modalités selon un ordre croissant ou décroissant. 
