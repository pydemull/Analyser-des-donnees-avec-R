...
```{r grouped bars code, message = FALSE, fig.align="center", warning = FALSE}
...
```{r grouped bars figure, echo = FALSE}
g1 / g2
```
...
```{r fig recap biv means, fig.align="center", echo = FALSE, fig.width=20}
library(patchwork)
g1 | g2 | g3
```
...
```{r fig recap biv med, fig.align="center", echo = FALSE, fig.width=20}
library(patchwork)
g4 | g5 | g6
```
...
```{r fig recap biv distri, fig.align="center",  echo = FALSE, fig.width=20}
g7 | g8 | g9
```
...
```{r fig recap biv dens, fig.align="center", echo = FALSE, message = FALSE, warning = FALSE, fig.width = 20}
g13 | g14
```

Lorsque les données à visualiser sont pairées, c'est-à-dire qu'il s'agit des mêmes individus qui ont des données pour chaque modalité de la variable qualitative (e.g., suite à une même mesure qui aurait été réalisée dans différentes conditions), il est possible d'ajouter des éléments graphiques, telles que des lignes, de telle sorte à mettre davantage en évidence le fait que les données sont liées. Un exemple de graphique pour cela est montré ci-dessous avec le jeu de données `mice2` (intégré au package `datarium`). Ce jeu de données contient des données de poids de 10 souris évaluées avant et après un traitement.
...
```{r paired data, fig.align="center"}
mice2 %>%
...
Dans R, le $d_{s}$ de Cohen peut être calculé à l'aide de la fonction `cohens_d()` du package `effectsize`, qui nécessite d'être installé puis chargé avant d'être utilisé. Pour illustrer l'utilisation de cette fonction, on peut utiliser le jeu de données `ToothGrowth`, qui contient des données d'augmentation de la longueur d'odontoblastes (cellules reponsables de la croissance des dents) de cochons de Guinée (variable quantitative `len`), qui ont été supplémentés en vitamine C selon deux méthodes différentes (variable qualitative `supp`) : avec jus d'orange (modalité OJ), ou directement avec de l'acide ascorbique (modalité VC). (Il y a également des dosages différents indiqués dans le jeu de données, mais cette variable n'est pas prise en compte pour l'exemple.) 

```{r ToothGrowth view, echo = FALSE, warning = FALSE, fig.align="center"}
head(ToothGrowth)
...
Dans ce jeu de données, les données quantitatives à comparer (`len`) selon les modalités OJ et VC de la variable qualitative `supp` sont indépendantes car les individus qui ont suivi les deux traitements n'étaient pas les mêmes. On est donc bien dans une situation de type *between-subject design*. On peut alors calculer le $d_{s}$ de Cohen à l'aide de la fonction `cohens_d()` de la manière suivante : 
...
cohens_d(len ~ supp, data = ToothGrowth, paired = FALSE, pooled_sd = TRUE)
```

Dans cet exemple, on remarque qu'on a bien cherché à savoir comment les données de la variable `len` peuvent différer en fonction (`~`) des modalités de la variable `supp`. Si la fonction nous donne un résultat, il faut toutefois bien faire attention au sens du calcul qui a été réalisé. Configurée de la sorte, la fonction `cohens_d()` réalise la différence **modalité 1 - modalité 2**. Il faut donc savoir quelle est la modalité 1 et quelle est la modalité 2 dans la variable `supp` pour ensuite pouvoir interpréter le signe du résultat, qui est positif ici avec la valeur de 0.49. Pour ce faire, on peut utiliser la fonction `levels()` :

```{r levels function}
levels(ToothGrowth$supp)
```

L'ordre des modalités affichées nous indique que OJ est la modalité 1, et que VC est la modalité 2. Par conséquent, le $d_{s}$ de Cohen de 0.49 obtenu plus haut indique que le traitement OJ (jus d'orange) a eu un effet supérieur à celui du traitement VC (acide ascorbique). Cette interprétation devrait être en cohérence avec le graphique réalisé au préalable. Si l'on avait voulu avoir le calcul inverse (VC - OJ), il aurait  fallu reconfigurer l'ordre des modalités, par exemple à l'aide de la fonction `fct_relevel()` du package `forcats` comme montré à la fin du chapite 3.
...
Il existe également une autre approche pour interpréter une valeur de taille d'effet : l'approche *Common Language explanation* [@Lakens2013]. Cette approche consiste à faire le lien entre la valeur de la taille d'effet et des probabilités de rencontrer des valeurs similaires ou supérieures dans les groupes comparés. Par exemple, lorsqu'on obtient un $d_{s}$ de Cohen de 0.80, cela peut se traduire par le fait qu'il y a 71.4 % de chances qu'une personne prise au hasard dans le groupe avec la meilleure moyenne ait un score plus élevé qu'une personne qui serait prise au hasard dans le groupe avec la moyenne la plus basse des deux groupes. Kristoffer Magnusson a réalisé une page web qui permet d'utiliser l'approche *Common Language explanation* pour n'importe quelle valeur de taille d'effet dans le cadre d'une étude de type *between-subject design*. Jetez un oeil ici : https://rpsychologist.com/cohend.

**gs de Hedges**

Il a été rapporté, dans la littérature, qu'en réalité le $d_{s}$ de Cohen est relativement biaisé lorsqu'il s'agit d'estimer la taille d'un effet dans la population d'intérêt à partir d'échantillons de population (autrement dit, à l'échelle de nombreux échantillons, on sait qu'avec le $d_{s}$ de Cohen on aura en moyenne un écart entre l'effet qu'on a trouvé et l'effet réel qui existe dans la population). Cela sera d'autant plus vrai lorsque les calculs seront réalisés à partir de petits échantillons ($N$ < 20), selon Hedges et Okins (1985) cités par Lakens [-@Lakens2013]. En raison de cela, un autre indice statistique a été proposé pour corriger cette erreur systématique, à savoir le $g_{s}$ de Hedges :
...
```{r hedges g between}
hedges_g(len ~ supp, data = ToothGrowth, paired = FALSE, pooled_sd = TRUE)
```

Ici, la valeur ne change pas vraiment par rapport au cas précédent (d'autant plus qu'elle est arrondie), car l'effectif n'est pas si petit que cela. Pour interpréter sa valeur, il convient de faire à nouveau attention à la différence réellement utilisée, qui reste ici OJ - VC.

**Delta de Glass**

Dans certains cas, les écarts-types des groupes à comparer peuvent être très différents. On peut se trouver dans ce genre de situation lorsqu'on compare des données de progression entre un groupe entraîné ou traité, et un groupe contrôle, avec un écart-type qui risquerait d'être plus élevé dans le groupe entraîné/traité sous l'effet d'une réponse potentiellement très hétérogène à l'entraînement ou au traitement selon les individus. Dans ce genre de situation, des indices statistiques autres que le $g_{s}$ de Hedges mériteraient d'être calculées, telles que le $\Delta$ de Glass [@Lakens2013] :

$$\Delta = \frac{\overline{X_{1}} - \overline{X_{2}}} {s_{2}}$$


Le calcul du $\Delta$ de Glass est dans l'idée le même  que celui du $d_{s}$ de Cohen, sauf que la différence des moyennes des deux groupes n'est pas divisée par l'écart-type commun des deux groupes, mais par celui d'un seul des deux groupes, qui serait en principe celui qui représenterait la condition contrôle ou la condition de référence. Dans R, le $\Delta$ de Glass lié à un échantillon de population peut être calculé à l'aide de la fonction `glass_delta()` du package `effectsize`. Attention, l'écart-type utilisé dans le code suivant est celui de la variable quantitative associée à la modalité 2 de la variable qualitative, qui est toujours VC dans cet exemple.

```{r glass delta between}
glass_delta(len ~ supp, data = ToothGrowth)
```
...
```{r Blink reconfiguration, echo = FALSE, message = FALSE}
Blink2 <-
  Blink %>%
...
$$d_{av} = \frac{M_{diff}}{\sqrt{\frac{s_{1}^2 + s_{2}^2} {2}}}$$

Ce calcul reviendrait donc à diviser la moyenne des différences par la moyenne des écarts-types respectifs des deux groupes de données. Pour obtenir $d_{av}$, il est possible de repartir de la fonction `cohens_d()` et de sa configuration pour des groupes indépendants, mais en indiquant `pooled_sd = FALSE` dans la fonction, comme ci-dessous : 

```{r cohens dav within, warning=FALSE, message=FALSE}
cohens_d(Blink_rate ~ Condition, data = Blink2, paired = FALSE, pooled_sd = FALSE)
```

En effet, que l'on fasse la moyenne des différences (ce qui serait attendu pour le présent calcul) ou la différence entre deux moyennes (ce qui est finalement fait par la fonction tel que configurée ci-dessus), cela ne change rien avec des données pairées. Le numérateur $M_{diff}$ reste donc correctement calculé même avec cette configuration de fonction qui est faite initialement pour des groupes indépendants. Le fait d'indiquer `pooled_sd = FALSE` change le calcul du dénominateur que nous avions rencontré initialement pour le calcul de $d_{s}$, cela pour obtenir le calcul attendu pour le $d_{av}$.

**gav de Hedges**

Comme dans le cas de l'étude de groupes indépendants, $d_{av}$ et ses dérivés sont biaisés. Il peut donc être préfarable à nouveau de calculer le $g_{av}$ de Hedges avec la fonction `hedges_g()` configurée comme ceci : 
