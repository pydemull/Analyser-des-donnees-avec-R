...
flextable(
  ...
  set_caption("Calcul de la covariance")
```
...
flextable(
  ...
  hline_top(part = "all", border = fp_border(width = 1.5)) %>%
  hline_bottom(part = "all", border = fp_border(width = 1.5)) %>%
  set_caption("Valeurs de taille d'effet associées aux valeurs de corrélation")
```
...
flextable(
 mtcars %>%
  ...
  hline_top(part = "all", border = fp_border(width = 1.5)) %>%
  hline_bottom(part = "all", border = fp_border(width = 1.5)) %>%
  set_caption("Les rangs d'une variable")
```
...
# Reconfiguration de l'ordre des modalités de la variable opinion, et calcul des 
# effectifs totaux pour les catégories étudiées
...
```{r taballUCBA, message=FALSE, echo = FALSE, fig.cap= "Taux de réussite des étudiants femmes et hommes à l'Université de Berkeley en 1973, approche globale [@bickelSexBiasGraduate1975]"}
recap_univ <- 
  results_univ %>%
...
Toutefois, une analyse par département permettait de voir que les taux de réussite des femmes étaient en réalité supérieurs voire nettement supérieurs à ceux des hommes dans la plupart des départements, comme montré ci-dessous.

```{r tabdepUCBA, echo = FALSE, message=FALSE, fig.height = 14, fig.cap= "Taux de réussite des étudiants femmes et hommes à l'Université de Berkeley en 1973, approche par département [@bickelSexBiasGraduate1975]"}
recap_dep <- 
  results_univ %>%
...
Cette situation, qui peut paraître étonnante, illustre ce qu'on appelle un paradoxe de Simpson, c'est-à-dire le fait que la conclusion issue d'une analyse globale peut se retrouver annulée voire inversée lors d'une analyse différenciée par groupe et sous-groupe. Dans le cas présent, l'apparent paradoxe s'explique par le fait que, contrairement aux hommes, la majorité des femmes avaient candidaté dans des départements qui étaient très sélectifs, c'est-à-dire où le taux de réussite était faible (il l'était aussi pour les hommes). Très peu de femmes avaient candidaté là où les taux de réussite étaient très élevés (pour les femmes comme pour les hommes). Ceci est illustré sur la figure ci-dessous. On voit bien que la majorité des femmes étaient inscrites dans les départements ici montrés avec les lettres allant de C à F, or ces départements étaient associés à des taux de réussite inférieurs à 40 % seulement, que cela soit pour les hommes ou pour les femmes (cf. Figure \@ref(fig:tabdepUCBA)).

```{r distriUCBA, echo = FALSE, message=FALSE, fig.cap= "Distribution des étudiants femmes et hommes par département à l'Université de Berkeley en 1973 [@bickelSexBiasGraduate1975]"}
recap_dep2 <- 
  results_univ %>%
  dplyr::select(-result) %>%
...
##################################################################
# Paramétrage de la taille de certains labels des figures à suivre
##################################################################
...
# Note : L'installation du package Hmisc est nécessaire pour pouvoir utiliser la 
# valeur "mean_sdl" avec l'argument "fun.data" dans les blocs de code ci-dessous)

#########################################
# Graphiques pour les données non pairées
#########################################

# Moyennes et barres d'erreur, et points avec mouvement latéral aléatoire
A <- 
...
# Aires montrant les densités de probabilité, et points avec mouvement latéral aléatoire
C <- 
  ggplot(data = iris, aes(x = Species, y = Sepal.Length)) +
  geom_point(aes(x = stage(start = Species, after_scale = x - 0.1)),
             position = position_jitter(seed = 123, width = 0.08, height = NULL), 
...
#####################################
# Graphiques pour les données pairées
#####################################

# Moyennes et barres d'erreur, et points avec mouvement latéral aléatoire 
D <-
ggplot(data = Blink2, aes(x = Condition, y = Blink_rate)) +
  geom_point(position = position_jitter(seed = 123, width = 0.05),
             size = 4, alpha = 0.3) +
  geom_line(aes(group = Subject), 
            position = position_jitter(seed = 123, width = 0.05),
            size = 2, colour = "gray", alpha = 0.3) +
  stat_summary(data = Blink2 %>% filter(Condition == "Straight"), 
               geom = "errorbar",
               fun.data = "mean_sdl", 
               fun.args = list(mult = 1),
               position = position_nudge(x = -0.1),
               size = 1.3, width = 0.1) +
  stat_summary(data = Blink2 %>% filter(Condition == "Oscillating"), 
               geom = "errorbar", 
               fun.data = "mean_sdl", 
               fun.args = list(mult = 1),
               position = position_nudge(x = 0.1),
...
ggplot(data = Blink2, aes(x = Condition, y = Blink_rate)) +
  ...
  geom_line(aes(group = Subject), 
            position = position_jitter(seed = 123, width = 0.05),
            size = 2, colour = "gray", alpha = 0.3) +
  geom_boxplot(data = Blink2 %>% filter(Condition == "Straight"), 
               width = 0.08, outlier.shape = NA,
               position = position_nudge(x = -0.15)) +  
  geom_boxplot(data = Blink2 %>% filter(Condition == "Oscillating"), 
               width = 0.08, outlier.shape = NA,
               position = position_nudge(x = 0.15)) + 
  ggtitle("E") +
  set_text_size
...
ggplot(data = Blink2, aes(x = Condition, y = Blink_rate)) +
  ...
  geom_line(aes(group = Subject), 
            position = position_jitter(seed = 123, width = 0.05),            
            size = 2, colour = "gray", alpha = 0.3) +
  geom_half_violin(data = Blink2 %>% filter(Condition == "Straight"),
                   side = "l", 
                   position = position_nudge(x = -0.1, y = 0)) +  
  geom_half_violin(data = Blink2 %>% filter(Condition == "Oscillating"), 
                   side = "r", 
                   position = position_nudge(x = 0.1, y = 0)) +  
  ggtitle("F") +
  set_text_size
...
####################################################
# Sources ayant aidé à la réalisation des graphiques
####################################################

# Pour les graphiques avec boxplots : https://github.com/tidyverse/ggplot2/issues/4491
# Pour les graphiques avec densité de probabilité : https://github.com/jorvlan/raincloudplots
```

```{r graphDistriBivQuantvsQual, fig.align="center", echo = FALSE, fig.width = 15, fig.height=20, fig.cap="Méthodes classiques pour montrer des données quantitatives en fonction des modalités d'une variable qualitative : données non pairées (A à C) et données pairées (D à F)"}
library(patchwork)
(A | D) / (B | E) / (C | F)
```

On remarque que chaque ligne de la Figure \@ref(fig:graphDistriBivQuantvsQual) contient des graphiques illustrant des éléments différents pour résumer la distribution : la moyenne et l'écart-type pour la première ligne, la boîte à moustaches pour la seconde ligne, et une aire montrant la densité de probabilité pour la troisième ligne. 
La moyenne et l'écart-type sont intéressants à montrer lorsque les analyses inférentielles réalisées dans l'étude consistent en la comparaison de moyennes avec l'acceptation de l'hypothèse selon laquelle les distributions des variables comparées sont gaussiennes et présentent des variances similaires. Toutefois, la moyenne et l'écart-type seuls sont problématiques car ils peuvent induire en erreur sur la manière dont sont réellement distribuées les observations. D'ailleurs, deux mêmes couples de moyennes et écart-types peuvent résulter de distributions très différentes [@weissgerberBarLineGraphs2015]. La boîte à moustaches, elle, a le mérite de refléter plus fiablement la distribution des données et de ne pas souffrir du risque d'être modifiée en présence de valeurs aberrantes. L'idée qu'on peut se faire des distributions et des comparaisons des variables est donc en principe plus robuste (i.e., moins sujette à variation d'une étude à une autre). Enfin, l'aire montrant la densité de probabilité est intéressante car elle décrit le plus précisément la distribution des observations. Cependant, sa forme et l'idée qu'elle donne de la distribution dans la population d’intérêt dépend beaucoup du nombre d'observations disponibles. Son utilisation pourrait donc se discuter en présence de peu d'observations. Quel que soit l'outil statistique et graphique utilisé pour caractériser la distribution, tous les graphiques présentés ci-dessus montrent les valeurs individuelles. En cela, ces graphiques vont dans le sens du propos de Weissgerber [-@weissgerberBarLineGraphs2015] qui milite pour la disparition des graphiques en forme de bâtons de dynamite, lesquels sont souvent utilisés pour montrer des moyennes et écart-types. La raison est que ces graphiques en forme de bâtons de dynamite peuvent induire en erreur quant à la réelle forme de la distribution et ils limitent les possibilités du lecteur de juger de la pertinence des choix d'analyse qui seraient faits par la suite sur la base de la distribution des observations. Remarquons par ailleurs que pour les graphiques de droite (ceux avec données pairées), on s'est attaché à montrer les lignes matérialisant le lien entre chaque donnée appartenant à un même individu. Cette pratique est intéressante car elle permet de juger, au-delà de l'évolution de la moyenne du groupe d'une condition à l'autre, dans quelle mesure les individus diffèrent dans leur évolution entre les deux conditions testées.

Pour pouvoir faire ces graphiques, il a fallu user de plusieurs nouvelles techniques : 

- Pour montrer les moyennes, il a fallu utiliser la fonction `stat_summary()`. Cette fonction sert à visualiser un résumé statistique identifié grâce à l'argument `fun`. On peut choisir sa forme avec l'argument `geom`.
- Pour montrer les écarts-types, il a fallu là aussi utiliser la fonction `stat_summary()`, cette fois en utilisant l'argument `fun.data` avec la valeur `mean_sdl`, et l'argument `fun.args` que l'on a configuré pour déterminer le nombre d'écart-types que la longueur de la barre d'erreur devait représenter (ici, c'est une fois l'écart-type, obtenu avec `mult = 1`).
- Pour montrer les aires représentant les densités de probabilité, il a fallu utiliser la fonction `geom_half_violin()` du package `gghalves`. L'argument `side`permet de choisir de quel côté l'on veut que l'aire apparaisse (à droite ou à gauche quand on est à la verticale).
- Pour montrer les données individuelles sans risquer qu'elle ne se superposent de trop, on a utilisé la fonction `geom_point()`, l'argument `position` et la fonction `position_jitter()`. A l'intérieur de la fonction `position_jitter()`, l'argument `seed` (la graine en français) désigne le nombre sur lequel se base l'ordinateur pour générer un résultat pseudo-aléatoire. Quand on regarde le graphique, les données individuelles apparaissent décalées aléatoirement sur le plan latéral. Cet effet aléatoire est l'intérêt même d'une fonction dite `jitter()`, mais cet effet aléatoire est en réalité prédéfini de telle sorte à ce que si on fixe la graine (`seed`), alors on obtiendra toujours le même effet aléatoire sur le graphique. Ceci est très important car cela permet d'aligner les lignes individuelles (cf. graphiques pour données pairées) avec les données (les points) individuelles, cela en configurant de manière identique l'argument `seed`.
- Pour mettre en vis-à-vis les données individuelles avec l'élément statistique dans les cas de données non pairées, la technique a consisté à décaler latéralement les données individuelles vers la gauche à l'aide de la fonction `stage()`, et à décaler latéralement l'élément statistique vers la droite à l'aide de l'argument `position` et de la fonction `position_nudge()`. A noter que l'argument `alpha`permet de rajouter de la transparence aux points pour permettre de voir là où les données se superposeraient davantage, ce qui est difficilement évitable quand il y a beaucoup de données.
- Pour mettre en vis-à-vis les données individuelles avec l'élément statistique dans les cas de données pairées, la technique a consisté à laisser les données individuelles centrées sur les axes relatifs à chacune des deux conditions testées, cela avec un mouvement aléatoire identique imposé aux données individuelles et aux lignes individuelles (l'argument `width` fixe l'importance du mouvement latéral aléatoire). De plus, il a fallu décaler latéralement l'élément statistique soit vers la droite, soit vers la gauche selon la condition concernée, cela à l'aide de l'argument `position` et de la fonction `position_nudge()`. Pour pouvoir appliquer un mouvement latéral différent selon la condition concernée, il a fallu proposer un morceau de code (avec `stat_summary()` ou `geom_boxplot()` ou `geom_half_violin()`) spécifique à chacun des éléments statistiques à montrer, chose ayant été réalisée en indiquant la partie du jeu de données concernée à l'intérieur de la fonction générant l'élément statistique (e.g., `data =  Blink2 %>% filter(Condition == "Oscillating")`).

Récemment, il a été proposé dans la littérature d'user de graphiques appelés *rainclouds plots* qui combineraient les avantages de plusieurs techniques graphiques et statistiques et par là même pallieraient les manques ou défauts inhérents à chacune des techniques vues ci-dessus [@allenRaincloudPlotsMultiplatform2019]. Ces *rainclouds plots* utilisent : 

- Les données individuelles, qui permettent de bien voir les différences individuelles, les *outliers*, et des patterns de distribution inattendus ;
- Une aire montrant la densité de probabilité, qui permet de donner une idée relativement précise de la distribution (chose difficile à obtenir à partir seulement des observations individuelles) ; 
- Une boîte à moustaches, qui permet de se faire une idée résumée relativement robuste de la distribution dans la population d'intérêt (via la médiane et les quartiles) ; selon les cas, d'autres statistiques, comme la moyenne et l'écart-type, peuvent être utilisées à la place ou en plus de la boîte à moustaches [@allenRaincloudPlotsMultiplatform2019].

Les blocs de code ci-dessous et la Figure \@ref(fig:raindcloudplotsBiv) proposent plusieurs options d'utilisation du *raincloud plot*. 
...
# Raincloud plots basculés à la verticale
B <-
  ggplot(data = iris, aes(x = Species, y = Sepal.Length, color = Species, fill = Species)) +
  geom_point(aes(x = stage(start = Species, after_scale = x - 0.1)),
             position = position_jitter(seed = 123, width = 0.08, height = NULL),
...
# Raincloud plots avec aires montrant les densités de probabilité sur une même ligne
# horizontale
C <-
  ggplot(data = iris, aes(x = 0, y = Sepal.Length, color = Species, fill = Species)) +
...
D <-
  ...
  geom_half_violin(side = "r",
                   position = position_nudge(x = 0.1, y = 0),
                   size = 1, alpha = 0.4) +
  geom_boxplot(position = position_dodge(width = 0.15),
               width = 0.1, colour = "black", alpha = 0.3, outlier.shape = NA) +
  ggtitle("D") +
  set_text_size +
  theme(legend.position = "bottom",
...
# Raincloud plots classiques avec moyennes et écart-types et ligne reliant les moyennes
E <-
  ggplot(data = Blink2, aes(x = Condition, y = Blink_rate, 
  ...
  geom_half_violin(side = "r", 
                   ...
                   size = 1, alpha = 0.4) +  
  geom_boxplot(width = 0.08, colour = "black", alpha = 0.3, outlier.shape = NA) + 
  stat_summary(geom = "errorbar",
               fun.data = "mean_sdl",
               fun.args = list(mult = 1),
               position = position_nudge(0.1), 
...
# Raincloud plots avec moyennes et écart-types et ligne reliant les moyennes 
# basculés à la verticale
F <-
  ggplot(data = Blink2, aes(x = Condition, y = Blink_rate, 
                            colour = Condition, fill = Condition)) +
  geom_point(aes(x = stage(start = Condition, after_scale = x - 0.15)),
...
G <-
  ...
  geom_half_violin(aes(x = 2.17),
                   side = "r",
                   position = position_dodge(width = 0),
                   size = 1, alpha = 0.4) +  
  geom_boxplot(aes(x = 2.1),
               position = position_dodge(width = 0.1),
               width = 0.08, colour = "black", alpha = 0.3, outlier.shape = NA) +
  ggtitle("G") +
  set_text_size +
  theme(legend.position = "bottom",
        axis.title.y = element_blank(),
...
```{r raindcloudplotsBiv, fig.align="center", echo = FALSE,  fig.width = 20, fig.height = 15, fig.cap="Exemples de raindcloud plots pour des données non pairées (A à D) et pairées (E à H)"}
library(patchwork)
(A |B | C | D) / (E | F | G | H)
```
...
Dans R, le $d_{s}$ de Cohen peut être calculé à l'aide de la fonction `cohens_d()` du package `effectsize`, qui nécessite d'être installé puis chargé avant d'être utilisé. Pour illustrer l'utilisation de cette fonction, on peut utiliser le jeu de données `iris_two_species` crée par nos soins pour l'exemple à partir du jeu de données `iris` et qui contient notamment la variable quantitative `Sepal.Length`, et deux modalités de la variable qualitative `Species` (`setosa` et `versicolor`).

```{r iris2peciesview, warning = FALSE, fig.align="center", fig.cap="Visualisation des données de `iris_two_species`"}
...
Le $d_{s}$ de Cohen s'obtient alors de la manière suivante : 
...
Pour la suite de l'analyse, on peut, comme réalisé plus haut, reconfigurer le jeu de données `Blink` de telle sorte à bien avoir les conditions testées dans une même colonne et les valeurs correspondantes dans la colonne d'à côté, avec la modalité `Straight` en tant que première modalité à considérer dans les fonctions à suivre. Le nouveau jeu de données s'appelle alors `Blink2`.

```{r Blinkreconfiguration, echo = FALSE, message = FALSE, fig.align = "center", fig.cap="Données du jeu de données `Blink2`"}
Blink2 <-
  Blink %>%
  pivot_longer(cols = c(Straight, Oscillating), 
               names_to = "Condition", values_to = "Blink_rate") %>%
  mutate(Condition = fct_relevel(Condition, "Straight", "Oscillating"))
...
cohens_d(Blink_rate ~ Condition, data = Blink2, paired = TRUE, pooled_sd = TRUE)
```
